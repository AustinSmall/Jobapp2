document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    form.querySelectorAll('select, input[type="text"]').forEach(el => el.style.backgroundColor = '');
    ...
    // If none of the special names matched, do generic required validation:
    ...
    requiredFields.forEach(field => {
      if (field.type === "radio") {
        ...
        field.style.backgroundColor = '#c8f7c5';
      }
    });

    // Now handle radio groups only once per group
    radioNames.forEach(name => {
      const radios = form.querySelectorAll(`input[name="${name}"]`);
      const checked = form.querySelector(`input[name="${name}"]:checked`);
      radios.forEach(radio => {
        if (radio.parentElement) {
          radio.parentElement.style.backgroundColor = checked ? '#c8f7c5' : '#f7c5c5';
        }
      });
    ...
    // If all fields are filled, you can proceed with form submission here if needed
  });
});